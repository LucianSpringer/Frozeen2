...
import { LogisticsEngine } from '../src/core/LogisticsCore';
import { SecurityEngine } from '../src/core/SecurityHeuristics';
...
const Checkout: React.FC = () => {
  ...
  React.useEffect(() => {
    if (formData.city.length > 3) {
      const coords = LogisticsEngine.resolveCityCoordinates(formData.city);
      const vector = LogisticsEngine.resolveLogisticsVector(cart, coords);
      setShippingCost(vector.finalShippingCost);
      setLogisticsInfo(vector);
    }
  }, [formData.city, cart]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Calculate Total (ensure this logic matches your component)
    const currentTotal = subtotal + shippingCost;

    // 2. Run Security Check
    const risk = SecurityEngine.evaluateTransactionRisk(user, currentTotal, {
      name: formData.name,
      address: formData.address
    });

    // 3. Block Critical Fraud
    if (risk.riskLevel === 'CRITICAL') {
      alert(`TRANSAKSI DITOLAK: Terdeteksi aktivitas mencurigakan.\nKode: ${risk.flags.join(', ')}`);
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
              ...
              <div className="border-t border-slate-100 dark:border-slate-700 pt-4 space-y-2">
                ...
                {logisticsInfo && (
                  <div className="bg-slate-50 dark:bg-slate-700 p-3 rounded-lg text-xs space-y-1">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Jarak Gudang ({logisticsInfo.originNode})</span>
                      <span className="font-mono">{logisticsInfo.distanceKm} km</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Packaging</span>
                      <span className="font-mono">{logisticsInfo.packagingDetails.boxesUsed.length}x Cooler Box</span>
                    </div>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {logisticsInfo.packagingDetails.boxesUsed.map((box: string, idx: number) => (
                        <span key={idx} className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-[10px] border border-blue-200">{box}</span>
                      ))}
                    </div>
                  </div>
                )}
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-slate-400 mt-4">
              ...
              <span>Secured by SHE Protocol</span>
            </div>
          </div>
