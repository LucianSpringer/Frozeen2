...
import { Package, Users, DollarSign, TrendingUp, Plus, Check, Truck, XCircle, ShoppingBag, BellRing, Trash2, CheckCircle } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8 animate-fade-in-up">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
            <h3 className="font-bold mb-6 text-slate-900 dark:text-white flex items-center gap-2">
              <TrendingUp size={20} className="text-green-500" /> Analisis Penjualan (7 Hari Terakhir)
            </h3>
            <div className="flex items-end justify-between h-48 gap-2">
              {[45, 60, 35, 80, 55, 90, 75].map((h, i) => (
                <div key={i} className="w-full bg-slate-100 dark:bg-slate-700 rounded-t-lg relative group overflow-hidden">
                  <div
                    className="absolute bottom-0 left-0 right-0 bg-sky-500 hover:bg-sky-400 transition-all duration-500"
                    style={{ height: `${h}%` }}
                  ></div>
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none whitespace-nowrap">
                    Rp {(h * 150000).toLocaleString('id-ID')}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
            <h3 className="font-bold mb-4 text-slate-900 dark:text-white flex items-center gap-2">
              <BellRing size={20} className="text-red-500" /> Stok Menipis
            </h3>
            <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {products.filter(p => p.stock < 50).map(p => (
                <div key={p.id} className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-100 dark:border-red-900/30 transition hover:scale-[1.02]">
                  <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200 line-clamp-1">{p.name}</p>
                    <p className="text-xs text-red-600 dark:text-red-400">Sisa Stok: {p.stock}</p>
                  </div>
                  <button onClick={() => setActiveTab('products')} className="text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 px-2 py-1 rounded hover:bg-slate-50 dark:hover:bg-slate-700 transition">Restock</button>
                </div>
              ))}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {orders.map(order => {
                  ...
                  return (
                    <tr
                      key={order.id}
                      className={`cursor-pointer transition-all duration-500 ${isNew
